    ...
    if item.flag == 0:
        ...
        if item.aligned:
            ...
            img = face_preprocess.preprocess(
                img, bbox=item.bbox, landmark=item.landmark, image_size='%d,%d' % (args.image_h, args.image_w))
...
by traversing root folder and output to <prefix>.lst. Otherwise im2rec will read\
 <prefix>.lst and create a database at <prefix>.rec')
