...
def parse_args():
    ...
    parser.add_argument('--ckpt', type=int, default=default.ckpt,
                        help='checkpoint saving option. 0: discard saving. 1: \
                            save when necessary. 2: always save')
    parser.add_argument('--verbose', type=int, default=default.verbose,
                        help='do verification testing and model saving every \
                            verbose batches')
    parser.add_argument('--lr', type=float,
                        default=default.lr, help='start learning rate')
...
def get_symbol(args):
    ...
    if config.loss_name == 'softmax':  # softmax
        ...
        if config.loss_m1 != 1.0 or config.loss_m2 != 0.0 or \
            ...
            if config.loss_m1 == 1.0 and config.loss_m2 == 0.0:
                ...
                gt_one_hot = mx.sym.one_hot(
                    gt_label, depth=config.num_classes, on_value=s_m,
                    off_value=0.0)
                fc7 = fc7 - gt_one_hot
            else:
        ...
        positive = mx.symbol.slice_axis(
            nembedding, axis=0, begin=args.per_batch_size // 3,
            end=2 * args.per_batch_size // 3)
        negative = mx.symbol.slice_axis(
            nembedding, axis=0, begin=2 * args.per_batch_size // 3,
...
def train_net(args):
    ...
    prefix = os.path.join(args.models_root, '%s-%s-%s' %
                          (args.network, args.loss, args.dataset), 'model')
    ...
    def ver_test(nbatch):
        ...
        for i in xrange(len(ver_list)):
            (_, _, acc2, std2, xnorm, embeddings_list, val,
             val_std) = verification.test(ver_list[i], model,
                                          args.batch_size, 10, None, None)
